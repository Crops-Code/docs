...
description: Environment variables used by the ClusterCode orchestrator and agents.
---
...
The CLI stores configuration in `~/.clustercode/config.json`. The only user-configurable key is `WORKER_NAME`. Use `clustercode config set` to manage it.
