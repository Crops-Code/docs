...
Booting a fresh Windows DevBox is slow: ClusterCode runs Windows as a full VM (via [dockur/windows](https://github.com/dockur/windows)), and the unattended Windows install takes **20–30 minutes** on first boot. Then you still have to install your toolchain — Node, Git, an editor, your AI CLIs — every single time.
...
<Aside type="note">
  Windows DevBoxes and golden images require a worker that can host Windows VMs (nested virtualization / KVM), and building an image from a DevBox requires a **Pro or Enterprise** plan with the **owner**, **admin**, or **member** role.
</Aside>
